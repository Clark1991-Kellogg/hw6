...
//   // ⬇️ ⬇️ ⬇️
  ...
  let response = await fetch('https://api.themoviedb.org/3/movie/now_playing?api_key=789d6431ec6ed8b3e29367645e873c3f&language=en-US')
...
for(let i=0; i<movies.length; i++) {
  ...
  for(let x=0; x<results.length; x++) {
    ...
    let movieId = (`${json.results[x].id}`)  
    let posterUrl = (`${json.results[x].poster_path}`)

  console.log(movieId)
  console.log(posterUrl)

  document.querySelector('.movies').insertAdjacentHTML('beforeend', `
  <div id="movies-${movieId}" class="w-1/5 p-4">
...
//   // ⬇️ ⬇️ ⬇️
  ...
  for (let i=0; i<movies.length; i++){
    ...
    for (let x=0; x<results.length; x++){
      ...
      let movieId = (results[x].id)  

      document.querySelector(`#movies-${movieId}`).addEventListener('click', function(event){
